...
Examples:
  ...
  $0 --clean                                      # Delete intermediate files after run
...
build_snakemake_cmd() {
    ...
    # If run_satsuma_alignment is on, pass the max_parallel token pool so
    # Snakemake enforces the concurrency limit set in pipeline_paths.yaml.
    local run_satsuma max_parallel
    run_satsuma=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_SA'
import yaml, sys
def load(p):
    try:
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(str(cfg.get("run_stages", {}).get("run_satsuma_alignment", False)).lower())
PYEOF_SA
)
    if [ "$run_satsuma" = "true" ]; then
        max_parallel=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_MP'
import yaml, sys
def load(p):
    try:
        with open(p) as f: return yaml.safe_load(f) or {}
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(cfg.get("satsuma_align", {}).get("max_parallel", 1))
PYEOF_MP
)
        CMD+=("--resources=satsuma_jobs=${max_parallel}")
        log_info "Satsuma alignment: max ${max_parallel} job(s) in parallel (${max_parallel} × $(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_TH'
import yaml, sys
def load(p):
    try:
        with open(p) as f: return yaml.safe_load(f) or {}
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(cfg.get("satsuma_align", {}).get("threads", 40))
PYEOF_TH
) threads each)"
    fi

    # If run_lastz_alignment is on, pass the lastz_jobs token pool so
    # Snakemake enforces the concurrency limit set in pipeline_paths.yaml.
    local run_lastz lastz_parallel
    run_lastz=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_LZ'
import yaml, sys
def load(p):
    try:
        with open(p) as f: return yaml.safe_load(f) or {}
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(str(cfg.get("run_stages", {}).get("run_lastz_alignment", False)).lower())
PYEOF_LZ
)
    if [ "$run_lastz" = "true" ]; then
        lastz_parallel=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_LP'
import yaml, sys
def load(p):
    try:
        with open(p) as f: return yaml.safe_load(f) or {}
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(cfg.get("lastz_align", {}).get("max_parallel", 2))
PYEOF_LP
)
        CMD+=("--resources=lastz_jobs=${lastz_parallel}")
        log_info "LastZ alignment: max ${lastz_parallel} job(s) in parallel (${lastz_parallel} × $(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_LT'
import yaml, sys
def load(p):
    try:
        with open(p) as f: return yaml.safe_load(f) or {}
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(cfg.get("lastz_align", {}).get("threads", 8))
PYEOF_LT
) threads each)"
    fi
    ...
    # Use -- to terminate option parsing so $TARGET is treated as a positional
    # target rule, not as another value for --configfile (which is nargs='+'
    # in Snakemake 8+ and would otherwise swallow the target).
    CMD+=("--" "$TARGET")
}


#######################################
# Output cleanup (--clean flag)
#######################################
cleanup_outputs() {
    # Resolve base_output_dir from config files
    local out_base
    out_base=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF3'
import yaml, sys, os
...
cfg = load(sys.argv[1])
cfg.update(load(sys.argv[2]))
print(cfg.get("base_output_dir", "outputs"))
PYEOF3
)

    if [ -z "$out_base" ]; then
        log_warning "--clean: could not determine base_output_dir — skipping cleanup"
        return 0
    fi

    log_info "--clean: removing intermediate files under $out_base"

    # ── Patterns to remove ──────────────────────────────────────────────────
    # Exact file/dir names matched anywhere under base_output_dir
    local EXACT_NAMES=(
        "2bit"
        "chainNet"
        "chainPreNet"
        ...
        "brkfile2"
        "brkfile3"
        "EBR_configs_cloned.txt"
        "ebrs_processed.marker"
        "EBRs_split_done.txt"
        "EBRs_subdirs_overlap_done.txt"
        "ebr_stats.log"
        "getENRICH_lineages.log"
        "config2.json"
        "getENRICH_done.txt"
        "all_align.txt"
        "getENRICH_msHSBs.log"
        "inter-msHSBs.txt"
        "msHSBs_done.txt"
        "msHSBs_fasta_done.txt"
        "msHSBs_regions.bed"
        "jobs"
        "logs"
        "synteny_assign_done"
    ...
    # Glob patterns matched anywhere under base_output_dir
    # NOTE: "*_out" is intentionally NOT listed here — it is handled below
    # with path-aware logic so that only {synteny_results}/{sample}_out dirs
    # are removed, while {synteny_results}/{window_size}/{sample}_out dirs
    # (which are needed for downstream steps) are preserved.
    local GLOB_PATTERNS=(
        "all_nonoverlap_combined_*_reformatted.txt"
        "all_nonoverlap_combined_*.txt"
    )
    ...
    for name in "${EXACT_NAMES[@]}"; do
        ...
        done < <(find "$out_base" -name "$name" -print0 2>/dev/null)
    done

    # ── Remove glob-pattern matches ──────────────────────────────────────────
    for pattern in "${GLOB_PATTERNS[@]}"; do
        ...
        done < <(find "$out_base" -name "$pattern" -print0 2>/dev/null)
    done

    # ── Remove *_out dirs that are direct children of synteny_results only ───
    # This avoids removing {window_size}/{sample}_out subdirs, which must be
    # kept, while still cleaning up {synteny_results}/{sample}_out dirs.
    local synteny_results
    synteny_results=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_SR'
import yaml, sys, os
...
sr = cfg.get("synteny_results", "outputs/synteny_processing")
# sr may already be absolute, or relative — resolve against out_base if relative
if not os.path.isabs(sr):
    # Strip a leading "outputs/" prefix if present, then join with out_base
    if sr.startswith("outputs/"):
        sr = os.path.join(out_base, sr[len("outputs/"):])
    else:
        sr = os.path.join(out_base, sr)
print(sr)
PYEOF_SR
)
    if [ -n "$synteny_results" ] && [ -d "$synteny_results" ]; then
        # -maxdepth 1 ensures we only match immediate children, not deeper paths
        while IFS= read -r -d '' target; do
            if rm -rf "$target" 2>/dev/null; then
                (( removed++ )) || true
            else
        ...
        done < <(find "$synteny_results" -maxdepth 1 -name "*_out" -print0 2>/dev/null)
    else
        log_warning "--clean: synteny_results dir not found ($synteny_results) — skipping *_out cleanup"
    fi

    # ── Remove chain/net only as direct children of Ancestor_seq_recunstruction ──
    # These names also appear deeper in the tree (e.g. data/vaga/habrotrocha/chain,
    # data/vaga/habrotrocha/net) and must be preserved, so we cannot use a global
    # find. Only the top-level siblings are intermediate outputs safe to delete.
    local ancestor_dir
    ancestor_dir=$(python3 - "$PATHS_FILE" "$CONFIG_FILE" <<'PYEOF_AN'
import yaml, sys, os
...
cfg.update(load(sys.argv[2]))
out_base = cfg.get("base_output_dir", "outputs")
an = cfg.get("chainNet", {}).get("output_dir", "outputs/Ancestor_seq_recunstruction")
if not os.path.isabs(an):
    if an.startswith("outputs/"):
        an = os.path.join(out_base, an[len("outputs/"):])
    else:
        an = os.path.join(out_base, an)
print(an)
PYEOF_AN
)
    if [ -n "$ancestor_dir" ] && [ -d "$ancestor_dir" ]; then
        for name in chain net; do
            target="$ancestor_dir/$name"
            if [ -e "$target" ]; then
                if rm -rf "$target" 2>/dev/null; then
                    (( removed++ )) || true
                else
        ...
        log_warning "--clean: Ancestor_seq_recunstruction dir not found ($ancestor_dir) — skipping chain/net cleanup"
    fi

    echo
    log_success "Cleanup complete — $removed item(s) removed, $failed could not be removed"
}
