def _find_reference_fasta():
    """
    Locate the reference FASTA in satsuma_align.fasta_dir using reference_name.
    Tries extensions: .fa, .fna, .fasta
    Falls back to eba_format.reference_fasta if set and exists.
    """
    ...
    return config["eba_format"].get("reference_fasta", "")

if should_run_rule("eba_analysis"):
    rule generate_chr_size:
        """
        Generate chr_size.txt from the reference FASTA using seqkit.
        The reference FASTA is discovered automatically from satsuma_align.fasta_dir
        using reference_name (tries .fa / .fna / .fasta).
        Output format: <chrom_name>\t<length>  (tab-separated, no header)
        """
        input:
            ref_fasta = _find_reference_fasta()
        output:
            chr_size = config["eba_format"]["chr_size_file"]
        params:
            ref_name = config["reference_name"]
        log:
            "logs/generate_chr_size.log"
        shell:
            """
            set -euo pipefail
            mkdir -p "$(dirname {output.chr_size})"
            mkdir -p "$(dirname {log})"

            echo "=== Generating chr_size.txt from reference FASTA ===" | tee {log}
            echo "Reference FASTA : {input.ref_fasta}"  | tee -a {log}
            echo "Reference name  : {params.ref_name}"  | tee -a {log}
            echo "Output file     : {output.chr_size}"  | tee -a {log}

            # seqkit fx2tab -l -n outputs: <name>\t<length>  (no header)
            seqkit fx2tab --length --name "{input.ref_fasta}" \
                > "{output.chr_size}" \
                2>> {log}

            if [ ! -s "{output.chr_size}" ]; then
                echo "ERROR: chr_size.txt is empty — check seqkit is installed and FASTA is valid" >&2 | tee -a {log}
                exit 1
            fi

            N_CHROM=$(wc -l < "{output.chr_size}")
            echo "Wrote $N_CHROM entries to {output.chr_size}" | tee -a {log}
            echo "=== First 5 lines ===" | tee -a {log}
            head -5 "{output.chr_size}" | tee -a {log}

            echo -e "\\e[32;1m✔\\e[0m \\e[33;1mCompleted\\e[0m chr_size.txt generation ($N_CHROM entries)" >&2
            """
...
if should_run_rule("eba_analysis"):
    rule run_eba:
        input:
            ...
            # Generated by generate_chr_size rule
            chr_size_file = rules.generate_chr_size.output.chr_size
        output:
        ...
        params:
            ...
            chr_size_abs = lambda wildcards, input: os.path.abspath(input.chr_size_file)
        log:
            "logs/run_eba.log"
        shell:
            """
...
if should_run_rule("eba_analysis"):
    rule ebr_stats_report:
        """Generate EBR statistics report (text + HTML) from EBRs.txt."""
        input:
            ebrs_file    = rules.process_ebrs.output.ebrs_file,
            stats_script = os.path.join(workflow.basedir, config["scripts"], "ebr_stats.py")
        output:
            txt  = os.path.join(config["eba_format"]["ebrs_dir"], "EBRs_stats.txt"),
            html = os.path.join(config["eba_format"]["ebrs_dir"], "EBRs_stats.html")
        params:
            out_dir = config["eba_format"]["ebrs_dir"]
        log:
            os.path.join(config["eba_format"]["ebrs_dir"], "ebr_stats.log")
        shell:
            """
            mkdir -p {params.out_dir}

            if [ ! -f "{input.stats_script}" ]; then
                echo "Error: ebr_stats.py not found at {input.stats_script}" >&2 | tee {log}
                exit 1
            fi

            if [ ! -f "{input.ebrs_file}" ]; then
                echo "Error: EBRs.txt not found at {input.ebrs_file}" >&2 | tee {log}
                exit 1
            fi

            python3 "{input.stats_script}" "{input.ebrs_file}" "{params.out_dir}" > {log} 2>&1

            if [ ! -f "{output.txt}" ] || [ ! -f "{output.html}" ]; then
                echo "Error: stats output files were not created" >&2 | tee -a {log}
                exit 1
            fi

            echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m EBR statistics report" >&2
            """
...
if should_run_rule("eba_analysis"):
    rule extract_msHSBs_fasta:
        """
        Extract msHSBs sequences from the reference FASTA using coordinates
        in msHSBs.txt (chrom, start, end — 1-based, inclusive).
        Uses seqkit (preferred) or samtools faidx as fallback.
        Output: <mshsbs_dir>/msHSBs_sequences.fasta
        """
        input:
            msHSBs    = rules.generate_msHSBs.output.final,
            ref_fasta = _find_reference_fasta()
        output:
            fasta  = os.path.join(config["eba_format"]["mshsbs_dir"], "msHSBs_sequences.fasta"),
            marker = touch(os.path.join(config["eba_format"]["mshsbs_dir"], "msHSBs_fasta_done.txt"))
        params:
            outdir    = config["eba_format"]["mshsbs_dir"],
            bed_file  = os.path.join(config["eba_format"]["mshsbs_dir"], "msHSBs_regions.bed")
        log:
            "logs/extract_msHSBs_fasta.log"
        shell:
            """
            set -euo pipefail
            mkdir -p {params.outdir}

            echo "=== Extracting msHSBs sequences from reference FASTA ===" | tee {log}
            echo "msHSBs file : {input.msHSBs}"   | tee -a {log}
            echo "Reference   : {input.ref_fasta}" | tee -a {log}
            echo "Output FASTA: {output.fasta}"    | tee -a {log}

            # ── Convert msHSBs.txt (1-based inclusive) to 0-based BED ──────────
            # msHSBs.txt format: chrom<TAB>start<TAB>end  (1-based, inclusive)
            # BED format:        chrom<TAB>start<TAB>end  (0-based, half-open)
            # So BED_start = msHSBs_start - 1,  BED_end = msHSBs_end (unchanged)
            echo "Converting msHSBs coordinates to BED format..." | tee -a {log}
            awk 'NF>=3 && $0!~/^#/ {{print $1"\t"($2-1)"\t"$3}}' {input.msHSBs} \
                > {params.bed_file}
            NREGIONS=$(wc -l < {params.bed_file})
            echo "Regions to extract: $NREGIONS" | tee -a {log}

            if [ "$NREGIONS" -eq 0 ]; then
                echo "ERROR: No regions found in {input.msHSBs}" >&2 | tee -a {log}
                exit 1
            fi

            # ── Tool detection ───────────────────────────────────────────────
            SEQKIT=$(command -v seqkit  2>/dev/null || true)
            SAMTOOLS=$(command -v samtools 2>/dev/null || true)

            if [ -n "$SEQKIT" ]; then
                echo "Tool selected: seqkit ($SEQKIT)" | tee -a {log}
                seqkit version | tee -a {log}

                # seqkit subseq --bed  expects 1-based BED (its own format),
                # but seqkit subseq --region works per-region.
                # The cleanest cross-version approach: seqkit subseq --bed
                # with a standard 0-based BED file — seqkit treats BED as
                # 0-based half-open natively.
                seqkit subseq \
                    --bed {params.bed_file} \
                    {input.ref_fasta} \
                    -o {output.fasta} \
            ...
            elif [ -n "$SAMTOOLS" ]; then
                echo "Tool selected: samtools ($SAMTOOLS)" | tee -a {log}
                samtools version | head -1 | tee -a {log}

                # Index the reference FASTA if .fai is missing
                if [ ! -f "{input.ref_fasta}.fai" ]; then
                    echo "Indexing reference FASTA with samtools faidx..." | tee -a {log}
                    samtools faidx {input.ref_fasta} 2>> {log}
                fi

                # samtools faidx takes region strings: chrom:start-end (1-based)
                # Build region list from the original msHSBs.txt (already 1-based)
                echo "Building samtools region list..." | tee -a {log}
                REGIONS=$(awk 'NF>=3 && $0!~/^#/ {{print $1":"$2"-"$3}}' {input.msHSBs})

                echo "Running samtools faidx..." | tee -a {log}
                samtools faidx {input.ref_fasta} $REGIONS \
                    > {output.fasta} \
                    2>> {log}

            else
                echo "ERROR: Neither seqkit nor samtools found in PATH." >&2 | tee -a {log}
                echo "Please install one of them (conda install -c bioconda seqkit  OR  conda install -c bioconda samtools)" >&2 | tee -a {log}
                exit 1
            fi

            # ── Verify output ────────────────────────────────────────────────
            if [ ! -s "{output.fasta}" ]; then
                echo "ERROR: Output FASTA is empty or was not created" >&2 | tee -a {log}
                exit 1
            fi

            N_SEQS=$(grep -c "^>" "{output.fasta}" || true)
            echo "Sequences written to output FASTA: $N_SEQS" | tee -a {log}

            if [ "$N_SEQS" -eq 0 ]; then
                echo "ERROR: No sequences in output FASTA — check chromosome names in msHSBs.txt match reference FASTA headers" >&2 | tee -a {log}
                exit 1
            fi

            echo "=== First 3 headers ===" | tee -a {log}
            grep "^>" "{output.fasta}" | head -3 | tee -a {log}

            echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m msHSBs FASTA extraction ($N_SEQS sequences)" >&2
            """
