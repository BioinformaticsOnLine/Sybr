...
if should_run_rule("enrichment_analysis"):
    rule msHSBs_background:
        ...
        shell:
            ...
            # Check if KEGG file is required and exists
            if [ "{params.resource}" = "ko" ] && [ ! -f "{input.kegg}" ]; then
                echo "ERROR: KEGG file required but not found: {input.kegg}" | tee -a {log}
                exit 1
            fi
            ...
            # Step 4: Create background list based on resource type
            if [ "{params.resource}" = "ko" ]; then
                echo "Resource is KEGG (ko), copying KEGG annotation file..." | tee -a {log}
                cp "{input.kegg}" "{output.copied_kegg}"
                echo "KEGG file copied: {output.copied_kegg}" | tee -a {log}
                
                echo "Creating background from KEGG annotation..." | tee -a {log}
                ...
                echo "Background from KEGG created: {output.background}" | tee -a {log}
            else
                echo "Resource is not KEGG, creating background from annotation file..." | tee -a {log}
...
if should_run_rule("enrichment_analysis"):
    rule clone_EBR_configs:
        input:
            ...
            all_ebrs_file = os.path.join(config["eba_format"]["ebrs_dir"], "EBRs.txt"),
        ...
        output:
            all_ebrs_config = os.path.join(ebrs_dir, "EBRs_Enrichment_results", "config2.json"),
...
#                "Cloning config files for EBRs and msHSBs enrichment analysis"
        ...
        echo "Error: msHSBs directory not found at {params.mshsbs_dir}" >&2 | tee {log}
            ...
            if ! python3 {input.script} {input.config_template} \
                {params.ebrs_dir}/EBRs_Enrichment_results \
...
if should_run_rule("enrichment_analysis"):
    rule run_getENRICH:
        ...
        output:
            ebrs_all_done = os.path.join(ebrs_dir, "EBRs_Enrichment_results", "getENRICH_done.txt"),
        ...
        params:
            ...
            all_ebrs_config_abs = lambda wc: os.path.abspath(os.path.join(ebrs_dir, "EBRs_Enrichment_results", "config2.json")),
            ...
            lineage_configs_abs = lambda wc: [os.path.abspath(os.path.join(ebrs_dir, f"{lineage}_lineage", "config2.json")) 
                                            for lineage in [os.path.basename(d).replace("_lineage", "") 
                                                            for d in glob.glob(f"{ebrs_dir}/*_lineage")]],
        log:
            all_log = os.path.join(ebrs_dir, "EBRs_Enrichment_results", "getENRICH_all.log"),
            lineage_log = os.path.join(ebrs_dir, "getENRICH_lineages.log"),
            ms_log = os.path.join(config["enrichment"].get("msHSBs_dir", "msHSBs"), "getENRICH_msHSBs.log")
        threads: 40  # Allow parallel execution
#        message:
#            "Running getENRICH analysis for all EBRs, lineages and msHSBs in parallel"
        shell:
            """
            # Create log parent directories (Snakemake does not auto-create for multi-logs)
            mkdir -p "$(dirname {log.all_log})"
            mkdir -p "$(dirname {log.lineage_log})"
            mkdir -p "$(dirname {log.ms_log})"
            ...
            # Run all-EBRs enrichment analysis in background
            ...
            if [ ! -f "{output.ebrs_all_done}" ]; then
                echo "Error: EBRs_Enrichment_results completion file not found" >&2 | tee -a {log.all_log}
