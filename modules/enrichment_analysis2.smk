...
#                "Running getENRICH analysis for all EBRs, lineages and msHSBs in parallel"
            shell:
                ...
                # Function to run getENRICH with error handling - continue even if exit code is non-zero
                run_analysis() {{
                    config=$1
                    outfile=$2
                    logfile=$3
                    echo "Starting analysis for $config" | tee -a "$logfile"
                    echo "Working directory: $(pwd)" | tee -a "$logfile"
                    echo "Config path: $config" | tee -a "$logfile"
                    echo "Resource: {params.resource}" | tee -a "$logfile"
                    ...
                    if [ -d "$result_dir" ] && [ -n "$(ls -A "$result_dir" 2>/dev/null)" ]; then
                        echo "Enrichment results successfully generated in $result_dir" | tee -a "$logfile"
                        touch "$outfile"
#                        echo "Completed analysis for $config" | tee -a "$logfile"
                        return 0
                    else
                        echo "Warning: No enrichment results found in $result_dir" | tee -a "$logfile"
                        return 1
                    fi
                }}
                ...
                echo "Running all analyses in parallel..." | tee -a {log.all_log}
                
                # Run all_EBRs analysis in background
                run_analysis "{params.all_ebrs_config_abs}" "{output.ebrs_all_done}" "{log.all_log}" &
                pid_all=$!

                echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m pathway enrichment analysis of all EBRs" >&2
                
                # Run lineage analyses in background

                echo -e "\e[32;1m✔\e[0m \e[33;1mStarteded\e[0m pathway enrichment analysis of all lineage specific EBRs" >&2
                ...
                for config_path in {params.lineage_configs_abs}; do
                    ...
                    echo "Starting analysis for $lineage_dir" | tee -a {log.lineage_log}
                    run_analysis "$config_path" "$outfile" "{log.lineage_log}" &
                    pids+=($!)
                done

                echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m pathway enrichment analysis of all lineage specific EBRs" >&2
                
                # Run msHSBs analysis in background
                run_analysis "{params.ms_config_abs}" "{output.mshsbs_done}" "{log.ms_log}" &
                pid_ms=$!
                
                echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m pathway enrichment analysis of all msHSBs" >&2

                # Wait for all background processes to complete
                wait $pid_all || echo "all_EBRs analysis completed with warnings" | tee -a {log.all_log}
                
                for pid in "${{pids[@]}}"; do
                    wait $pid || echo "Lineage analysis completed with warnings" | tee -a {log.lineage_log}
                done
                
                wait $pid_ms || echo "msHSBs analysis completed with warnings" | tee -a {log.ms_log}
