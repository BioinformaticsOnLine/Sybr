...
def create_synteny_assign_rules():
    ...
    for window_size in WINDOW_SIZES:
        ...
        rule_dict = {
            'input': lambda wildcards: f"{{synteny_results}}/{{wildcards.sample}}_out/st_input",
            'output': lambda wildcards: f"{{synteny_results}}/synteny_out/{window_size}/{{wildcards.sample}}_out/synteny_assign_done",
            'params': lambda wildcards: {
                ...
                'step_size': STEP_SIZE,
                'target_dir': os.path.dirname(f"{{synteny_results}}/synteny_out/{window_size}/{{wildcards.sample}}_out/synteny_assign_done"),
                'log_file': os.path.join(
                    os.path.dirname(f"{{synteny_results}}/synteny_out/{window_size}/{{wildcards.sample}}_out/synteny_assign_done"),
                    "logs", "synteny_assign.log"
                )
            },
            'log': lambda wildcards: f"{{synteny_results}}/synteny_out/{window_size}/{{wildcards.sample}}_out/logs/synteny_assign.log",
            'shell': f"""
...
# Actually, let's use a simpler approach with a single rule that uses wildcards for window size
if should_run_rule("synteny_processing"):
    rule synteny_assign:
        input:
            st_input = "{synteny_results}/{sample}_out/st_input"
        output:
            done_file = "{synteny_results}/synteny_out/{window_size}/{sample}_out/synteny_assign_done"
        params:
            script_path = os.path.abspath("script_base/synteny_assign_v3_80_genes.pl"),
            ref_name = config["reference_name"],
        ...
        log:
            "{synteny_results}/synteny_out/{window_size}/{sample}_out/logs/synteny_assign.log"
        shell:
            """
            # STAGE 1: SETUP - Use absolute paths everywhere
            TARGET_DIR="{params.target_dir}"
            ...
            # STAGE 3: EXECUTION
            cd "$TARGET_DIR" && \
            if ! perl "$SCRIPT_PATH" {params.ref_name} {params.sample_name} {params.window_size} {params.step_size} st_input > "{params.log_file}" 2>&1; then
                echo "----------------------------------------" >&2
                echo "SYNTENY ASSIGNMENT FAILURE DETAILS" >&2
                echo "Working directory: $(pwd)" >&2
                echo "Command: perl $SCRIPT_PATH {params.ref_name} {params.sample_name} {params.window_size} {params.step_size} st_input" >&2
                echo "Last 10 lines of log:" >&2
                tail -n 10 "{params.log_file}" >&2
                echo "----------------------------------------" >&2
                exit 1
            ...
            echo -e "\e[32;1m✔\e[0m \e[33;1mCompleted\e[0m synteny tracking of \e[36;1m{wildcards.sample}\e[0m in {params.window_size} resolution" >&2
            """
...
if should_run_rule("synteny_processing"):
    rule generate_linear_synteny_plots:
        input:
            # Depend on species splitting to complete
            split_marker = rules.split_by_species.output.marker
        output:
            # Output marker file to indicate completion
            marker = touch(os.path.join(SYNTENY_RESULTS, ".linear_synteny_plots_done"))
        params:
            # Get sequence lengths file from config or use default
            sequence_lengths_file = lambda wildcards: config.get("linear_synteny", {}).get("sequence_lengths_file", 
                os.path.join(workflow.basedir, "inputs", "seq2", "all_sequence_lengths.txt")),
            output_dir = os.path.join(SYNTENY_RESULTS, config["out_final"]),
            # Use the actual filename linear_synteny.R
            script_path = lambda wildcards: os.path.join(SCRIPTS, "linear_synteny.R"),
            # Get plot directory from config or use default
            plot_dir = lambda wildcards: os.path.join(SYNTENY_RESULTS, config["out_final"], 
                config.get("linear_synteny", {}).get("plot_subdir", "linear_synteny_plots"))
        log:
        ...
        shell:
            ...
            echo "Starting linear synteny plot generation..." > {log}
            echo "Date: $(date)" >> {log}
            ...
            if [ ! -f "{params.sequence_lengths_file}" ]; then
                echo "Error: Sequence lengths file not found: {params.sequence_lengths_file}" >&2 | tee -a {log}
                exit 1
            fi
            
            if [ ! -f "{params.script_path}" ]; then
                echo "Error: R script not found: {params.script_path}" >&2 | tee -a {log}
                exit 1
            fi
            ...
            SCRIPT_ABS_PATH="$(cd "$(dirname "{params.script_path}")" && pwd)/$(basename "{params.script_path}")"
            
            echo "Parameters:" >> {log}
            echo "  R script: $SCRIPT_ABS_PATH" >> {log}
            echo "  Sequence lengths: {params.sequence_lengths_file}" >> {log}
            echo "  Output directory: {params.output_dir}" >> {log}
            echo "  Plot directory: $plot_dir" >> {log}
            
            # Check if split_by_species directories exist
            echo -e "\nChecking for split_by_species directories..." >> {log}
            split_dirs=$(find "{params.output_dir}" -name "split_by_species" -type d 2>/dev/null || true)
            
            if [ -n "$split_dirs" ]; then
                echo "Found split_by_species directories:" >> {log}
                echo "$split_dirs" >> {log}
                
                # Count species files
                species_files=$(find "{params.output_dir}" -path "*/split_by_species/*.txt" -type f 2>/dev/null | wc -l)
                echo "Total species files found: $species_files" >> {log}
                
                if [ "$species_files" -eq 0 ]; then
                    echo "Warning: No species-specific files found. R script may not have expected input." >> {log}
                fi
            else
                echo "Warning: No split_by_species directories found." >> {log}
                echo "R script may be looking for files in different location." >> {log}
            fi
            
            # Check for reformatted files (backup input for R script)
            echo -e "\nChecking for reformatted files..." >> {log}
            reformatted_files=$(find "{params.output_dir}" -name "*_reformatted.txt" -type f 2>/dev/null || true)
            
            if [ -n "$reformatted_files" ]; then
                reformatted_count=$(echo "$reformatted_files" | wc -l)
                echo "Found $reformatted_count reformatted files:" >> {log}
                echo "$reformatted_files" | head -10 >> {log}
                if [ "$reformatted_count" -gt 10 ]; then
                    echo "... and $((reformatted_count - 10)) more" >> {log}
                fi
            else
                echo "No reformatted files found." >> {log}
            fi
            
            # Run the R script - Use absolute path to script
            echo -e "\nRunning R script..." >> {log}
            echo "Command: Rscript \\"$SCRIPT_ABS_PATH\\" \\"{params.sequence_lengths_file}\\" \\"{params.output_dir}\\"" >> {log}
            
            cd "$plot_dir" && \
            if ! Rscript "$SCRIPT_ABS_PATH" "{params.sequence_lengths_file}" "{params.output_dir}" 2>&1 | tee -a {log}; then
                echo "Error: linear_synteny.R failed" >&2 | tee -a {log}
                echo "Check R error messages above." >> {log}
                exit 1
            fi
            
            # Check if any plot files were created
            echo -e "\nChecking for generated plot files..." >> {log}
            plot_count=0
            for ext in pdf png jpg jpeg svg tiff; do
                ext_count=$(find "$plot_dir" -name "*.$ext" -type f 2>/dev/null | wc -l)
                if [ "$ext_count" -gt 0 ]; then
                    echo "  Found $ext_count .$ext files" >> {log}
                    plot_count=$((plot_count + ext_count))
                fi
            done
            
            if [ "$plot_count" -eq 0 ]; then
                echo "Warning: No plot files found in $plot_dir" >> {log}
                echo "Checking directory contents..." >> {log}
                ls -la "$plot_dir/" >> {log} 2>&1 || true
            else
                echo "Total plot files created: $plot_count" >> {log}
                
                # List first 10 plot files
                echo "First 10 plot files:" >> {log}
                find "$plot_dir" -name "*.pdf" -o -name "*.png" -o -name "*.jpg" -o -name "*.jpeg" -o -name "*.svg" -o -name "*.tiff" 2>/dev/null | \
                    head -10 >> {log}
                
                if [ "$plot_count" -gt 10 ]; then
                    echo "... and $((plot_count - 10)) more files" >> {log}
                fi
            fi
            
            # Check for other output files (CSV, TXT, etc.)
            echo -e "\nChecking for other output files..." >> {log}
            other_files=$(find "$plot_dir" -type f ! -name ".*" 2>/dev/null | grep -E "\.(csv|txt|tsv|log)$" || true)
            if [ -n "$other_files" ]; then
                other_count=$(echo "$other_files" | wc -l)
                echo "Found $other_count other output files (CSV, TXT, etc.)" >> {log}
            fi
            
            # Create a marker file to indicate completion
            touch "{output.marker}"
            
            echo -e "\n=== Linear Synteny Plot Generation Summary ===" >> {log}
            echo "Output directory: $plot_dir" >> {log}
            echo "Plot files created: $plot_count" >> {log}
            echo "Other files: $([ -n "$other_files" ] && echo "$other_count" || echo "0")" >> {log}
            echo "Completion time: $(date)" >> {log}
            
            echo "Linear synteny plot generation completed successfully" >> {log}
