...
num_terms <- nrow(padjust_filtered_data)
...
png("padjust_upset_plot.png", width = plot_width, height = plot_height, res = 300)
upset(
  upset_data,
...
pdf("padjust_upset_plot.pdf", width = plot_width / 300, height = plot_height / 300)  # Convert dimensions to inches
upset(
  upset_data,
  sets = colnames(upset_data)[-1],
  order.by = "freq",
