...
library(circlize) # Added to use colorRamp2
...
# Debug: Inspect column names in the data frame
...
# Debug: Inspect the filtered data
#print("Filtered data:")
#print(head(padjust_filtered_data))

# Extract gene lists from `geneID` column and create a term-gene mapping
...
# Debug: Inspect gene-term data frame
#print("Gene-term mapping data frame:")
#print(head(gene_term_df))
...
# Save heatmap as PDF
pdf("padjust_heatmap_plot.pdf", height = heatmap_height, width = heatmap_width,
    paper = "special", pointsize = 10)
draw_heatmap()
dev.off()

# Save heatmap as PNG
png("padjust_heatmap_plot.png", height = heatmap_height, width = heatmap_width, units = "in", res = 300)
draw_heatmap()
dev.off()
