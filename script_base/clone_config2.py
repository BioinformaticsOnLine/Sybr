...
def create_modified_config(source_config_path, target_folder):
    """Create a modified config file in the target folder with relative paths"""
    try:
        ...
        # Modify input paths to use local filenames in target folder
        ...
        # Update output directory to be the target folder itself with new name
...
def main():
    parser = argparse.ArgumentParser(
        description="Clone config file to multiple folders with relative paths",
    ...
    for folder in args.target_folders:
        ...
        if create_modified_config(args.source_config, folder):
            success_count += 1
