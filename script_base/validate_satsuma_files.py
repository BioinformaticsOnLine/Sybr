...
# ─────────────────────────────────────────────────────────────────────────────
#  LAYER 0 — Input directory structure validation
# ─────────────────────────────────────────────────────────────────────────────
#
#  Required layout (fixed files/dirs are mandatory; variable-content dirs
#  only need ≥1 file of the right extension):
#
#  inputs/
#  ├── synteny_processing/          [synteny_processing stage]
#  │   ├── all_sequence_lengths.txt   (fixed)
#  │   ├── Scaffolds.txt              (fixed)
#  │   └── Satsuma_alignments/        (variable — ≥1 *.txt)
#  ├── eba_analysis/                [eba_analysis stage]
#  │   ├── chr_size.txt               (fixed)
#  │   └── classification.eba         (fixed)
#  ├── enrichment_analysis/         [enrichment_analysis stage]
#  │   ├── protein_annotation.tsv     (fixed)
#  │   └── 3kegg_annotationTOgenes.txt (fixed)
#  └── Ancestor_seq_recunstruction/ [chainNet_generation + Ancestor stage]
#      ├── species_info.txt           (fixed)
#      ├── tree.txt                   (fixed)
#      ├── seq/                       (variable — ≥1 *.fa / *.fasta / *.fna)
#      └── LastZ_alignments/          (variable — ≥1 *.axt)


def _tree_line(name, is_last):
    """Return the pretty-print prefix for a tree entry."""
    return ("└── " if is_last else "├── ") + name


def _print_structure_tree(base_input_dir, stage_checks):
    """
    Print ONLY the failing entries in the directory tree.
    Folders where all items passed are silently skipped.
    """
    by_folder = {}
    for item in stage_checks:
        by_folder.setdefault(item["folder"], []).append(item)

    failing_folders = {f: items for f, items in by_folder.items()
                       if any(not i["found"] for i in items)}

    if not failing_folders:
        return  # all passed — nothing to print

    print(f"\n  {BOLD}inputs/{NC}  ->  {CYAN}{base_input_dir}{NC}")

    folder_list = list(failing_folders.keys())
    for fi, folder in enumerate(folder_list):
        is_last_folder = (fi == len(folder_list) - 1)
        folder_prefix  = "\\-- " if is_last_folder else "|-- "
        child_indent   = "    " if is_last_folder else "|   "

        print(f"  {RED}{folder_prefix}{folder}/{NC}")

        fail_items = [i for i in failing_folders[folder] if not i["found"]]
        for ci, item in enumerate(fail_items):
            is_last_child  = (ci == len(fail_items) - 1)
            child_prefix   = "\\-- " if is_last_child else "|-- "
            extra = f"  {YELLOW}[{item['detail']}]{NC}" if item.get("detail") else ""
            print(f"  {child_indent}{child_prefix}{RED}✗{NC} {item['name']}{extra}")

def check_input_structure(cfg, run_stages):
    """
    Validate that the inputs/ directory contains the expected structure
    for every enabled pipeline stage.

    Returns (errors, stage_checks) where stage_checks is a flat list of
    dicts used by _print_structure_tree().
    """
    errors       = 0
    stage_checks = []   # list of {folder, name, found, detail}

    in_base = cfg.get("base_input_dir", "inputs")

    # ── helper lambdas ────────────────────────────────────────────────────────
    def record(folder, name, found, detail=""):
        nonlocal errors
        stage_checks.append({"folder": folder, "name": name,
                              "found": found,   "detail": detail})
        if not found:
            errors += 1

    def req_file(abs_path, folder, display_name):
        found = os.path.isfile(abs_path)
        record(folder, display_name, found,
               "" if found else f"missing: {abs_path}")

    def req_dir_with_files(abs_dir, folder, display_name, exts):
        """Directory must exist and contain ≥1 file with one of the given extensions."""
        if not os.path.isdir(abs_dir):
            record(folder, display_name, False, f"directory missing: {abs_dir}")
            return
        matched = []
        for ext in exts:
            matched += glob.glob(os.path.join(abs_dir, f"*{ext}"))
        if matched:
            record(folder, f"{display_name}/  ({len(matched)} {'/'.join(exts)} files)",
                   True)
        else:
            record(folder, display_name, False,
                   f"no {'/'.join(exts)} files found in {abs_dir}")

    # ── derive canonical input sub-dirs from config (fall back to defaults) ───
    synteny_dir  = cfg.get("satsuma_alignments",
                            os.path.join(in_base, "synteny_processing", "Satsuma_alignments"))
    seq_len_file = cfg.get("sequence_lengths_file",
                            os.path.join(in_base, "synteny_processing", "all_sequence_lengths.txt"))
    scaffolds    = cfg.get("eba_format", {}).get("scaffolds_file", "")
    # Scaffolds.txt path — normalise "ALL_CHROMOSOMES" sentinel
    if not scaffolds or scaffolds in ("ALL_CHROMOSOMES", "Chromosomes"):
        scaffolds_path = os.path.join(in_base, "synteny_processing", "Scaffolds.txt")
        scaffolds_is_sentinel = True
    else:
        scaffolds_path = scaffolds
        scaffolds_is_sentinel = False

    chr_size      = cfg.get("eba_format", {}).get("chr_size_file",
                             os.path.join(in_base, "eba_analysis", "chr_size.txt"))
    cls_file      = cfg.get("eba", {}).get("c",
                             os.path.join(in_base, "eba_analysis", "classification.eba"))
    ann_file      = cfg.get("enrichment", {}).get("annotation_file",
                             os.path.join(in_base, "enrichment_analysis", "protein_annotation.tsv"))
    kegg_file     = cfg.get("enrichment", {}).get("kegg_file",
                             os.path.join(in_base, "enrichment_analysis", "3kegg_annotationTOgenes.txt"))
    seq_dir       = cfg.get("chainNet", {}).get("seq_dir",
                             os.path.join(in_base, "Ancestor_seq_recunstruction", "seq"))
    lastz_dir     = cfg.get("chainNet", {}).get("lastZ_alignments",
                             os.path.join(in_base, "Ancestor_seq_recunstruction", "LastZ_alignments"))
    tree_file     = cfg.get("deschrambler", {}).get("tree_file",
                             os.path.join(in_base, "Ancestor_seq_recunstruction", "tree.txt"))
    species_file  = cfg.get("deschrambler", {}).get("species_file",
                             os.path.join(in_base, "Ancestor_seq_recunstruction", "species_info.txt"))

    # ── per-stage structure checks ────────────────────────────────────────────

    if run_stages.get("synteny_processing", False):
        folder = "synteny_processing"
        req_file(seq_len_file, folder, "all_sequence_lengths.txt")
        if scaffolds_is_sentinel:
            # Scaffolds.txt is recommended but pipeline also accepts ALL_CHROMOSOMES
            found = os.path.isfile(scaffolds_path)
            detail = "" if found else "optional — pipeline will use ALL_CHROMOSOMES"
            stage_checks.append({"folder": folder, "name": "Scaffolds.txt  (optional)",
                                  "found": True,    "detail": detail})
        else:
            req_file(scaffolds_path, folder, "Scaffolds.txt")
        req_dir_with_files(synteny_dir, folder, "Satsuma_alignments", [".txt"])

    if run_stages.get("eba_analysis", False):
        folder = "eba_analysis"
        req_file(chr_size,  folder, "chr_size.txt")
        req_file(cls_file,  folder, "classification.eba")
        ref_fasta_path = cfg.get("eba_format", {}).get("reference_fasta",
                                  os.path.join(in_base, "eba_analysis", "reference.fasta"))
        req_file(ref_fasta_path, folder, "reference.fasta  (for msHSBs sequence extraction)")

    if run_stages.get("enrichment_analysis", False):
        folder = "enrichment_analysis"
        req_file(ann_file,  folder, "protein_annotation.tsv")
        req_file(kegg_file, folder, "3kegg_annotationTOgenes.txt")

    if run_stages.get("chainNet_generation", False) or \
       run_stages.get("Ancestor_seq_recunstruction", False):
        folder = "Ancestor_seq_recunstruction"
        req_file(tree_file,    folder, "tree.txt")
        req_file(species_file, folder, "species_info.txt")
        req_dir_with_files(seq_dir,   folder, "seq",
                           [".fa", ".fasta", ".fna"])
        req_dir_with_files(lastz_dir, folder, "LastZ_alignments", [".axt"])

    return errors, stage_checks
...
def validate_eba(cfg):
    ...
    # ── reference FASTA for msHSBs sequence extraction ───────────────────────
    ref_fasta = eba_fmt.get("reference_fasta", "")
    if ref_fasta:
        if check_file(ref_fasta, "EBA/reference.fasta", min_lines=2):
            with open(ref_fasta, errors="replace") as f:
                first = f.readline().strip()
            if not first.startswith(">"):
                fail("EBA/reference.fasta: does not start with '>'  (not valid FASTA)")
                errors += 1
            else:
                n_headers = sum(1 for ln in open(ref_fasta, errors="replace")
                                if ln.startswith(">"))
                ok(f"EBA/reference.fasta  ({n_headers} sequence(s) found)")
        else:
            errors += 1
    else:
        warn("EBA reference_fasta not configured — msHSBs sequence extraction will be skipped")
...
def main():
    ...
    active = [s for s, v in run_stages.items() if v]
    if active:
        print(f"\n{BOLD}Active stages:{NC} " +
              ", ".join(f"{CYAN}{s}{NC}" for s in active))
    else:
        print(f"\n{YELLOW}⚠  No stages are enabled in run_stages.{NC}")

    # ── LAYER 0: input directory structure ────────────────────────────────────
    section("Input directory structure")
    struct_errors, stage_checks = check_input_structure(cfg, run_stages)
    _print_structure_tree(cfg.get("base_input_dir", "inputs"), stage_checks)
    section_result(struct_errors)
    total_errors += struct_errors

    if struct_errors:
        print(f"\n{RED}{BOLD}Resolve missing input files/directories above "
              f"before re-running.{NC}")
        print(f"\n{'═'*60}")
        print(f"{RED}✗ Structure check failed — {struct_errors} item(s) missing.{NC}")
        print(f"{'═'*60}\n")
        sys.exit(1)
